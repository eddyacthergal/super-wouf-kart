 ...
 * pause, résultats et erreurs. Paramètres d'URL : `?autopilot=1` (démo) et `?debug=1` (journal).
 */
@Component({
  selector: 'app-race-page',
  imports: [RaceHud, Countdown, RaceAnnouncer, RaceToolbar, PauseDialog, RaceResults, RaceError],
  template: `
    <main class="fixed inset-0 overflow-hidden bg-azure-200">
      ...
      @if (run() % 2 === 0) {
        <canvas #canvas class="fixed inset-0 block size-full" role="img" aria-label="Course en cours"></canvas>
      } @else {
        <canvas #canvas class="fixed inset-0 block size-full" role="img" aria-label="Course en cours"></canvas>
      }

      @if (visibleHud(); as hud) {
        <app-race-hud [hud]="hud" [info]="session.info()" />
      }
